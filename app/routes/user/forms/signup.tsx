...
export default function Signup({ stack, setVerifyEmail }: { stack: ReturnType<typeof useModalsStack<AuthModal>>, setVerifyEmail: React.Dispatch<React.SetStateAction<string>> }) {
  ...
  const form = useForm({
    mode: "uncontrolled", initialValues: { email: '', firstName: '', lastName: '', username: '', password: '', confirmPassword: '' },
    validate: {
  ...
  return (
    <div>
      <div className=''>
        <div className=" px-3">
          <div className="flex items-center justify-center gap-10"><div className="text-lime-dark text-center font-extrabold text-4xl md:text-5xl font-base w-3/4">Sign Up</div></div>
          ...
          <form onSubmit={form.onSubmit(HandleSubmission)}>
            ...
            <Forminput content="Email" error={form.errors.email?.toString() || ''}{...form.getInputProps("email")} placeholder='Email' />
